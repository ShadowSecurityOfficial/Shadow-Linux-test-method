...
sudo -E brl import /shadowtmp/kali-linux-2022.2-vmware-amd64.vmdk -n "kali"
sudo -E cat /shadow/etc/shadow.conf | sudo -E pcregrep -o '(?<=default = ).*(?=:/sbin/init)' | sudo -E sed -n 2p | sudo -E xargs -I {} sed -i 's/'{}'/parrot-home-mate/g' /shadow/etc/shadow.conf
sudo -E reboot
